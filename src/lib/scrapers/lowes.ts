...
export async function scrapeLowes(): Promise<RawDeal[]> {
  const sourceUrl = 'https://www.lowes.com/c/deals'
  const html = await fetchHTML(sourceUrl)
  const out: RawDeal[] = []

  const tilePattern = /<a[^>]+href="(\/pd\/[^"]+\/\d+)"[^>]*>([\s\S]{0,800}?)<\/a>/gi
  ...
  for (const tile of tiles) {
    ...
    out.push({
      ...
      product_url: path.startsWith('http') ? path : `https://www.lowes.com${path}`,
