...
export async function scrapeCanadianTire(): Promise<RawDeal[]> {
  const sourceUrl = 'https://www.canadiantire.ca/en/deals.html'
  const html = await fetchHTML(sourceUrl)
  const out: RawDeal[] = []

  const tilePattern = /<a[^>]+href="(\/en\/pdp\/[^"]+\.html)"[^>]*>([\s\S]{0,800}?)<\/a>/gi
  ...
  for (const tile of tiles) {
    ...
    out.push({
      retailer_slug: 'canadiantire',
      retailer_name: 'Canadian Tire',
      country: 'CA',
      source_url: sourceUrl,
      product_url: path.startsWith('http') ? path : `https://www.canadiantire.ca${path}`,
