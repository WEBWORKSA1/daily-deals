...
export async function scrapeHomeDepot(): Promise<RawDeal[]> {
  const sourceUrl = 'https://www.homedepot.com/c/SpecialBuysOfTheDay'
  const html = await fetchHTML(sourceUrl)
  const out: RawDeal[] = []

  const tilePattern = /<a[^>]+href="(\/p\/[^"]+\/\d+)"[^>]*>([\s\S]{0,800}?)<\/a>/gi
  ...
  for (const tile of tiles) {
    ...
    out.push({
      retailer_slug: 'homedepot',
      retailer_name: 'Home Depot',
      country: 'US',
      source_url: sourceUrl,
      product_url: path.startsWith('http') ? path : `https://www.homedepot.com${path}`,
